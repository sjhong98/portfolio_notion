export const projectList = [
    { 
      ...
      dev: ["카카오맵 API 및 행정구역 폴리곤 적용", "로딩 중 스켈레톤 UI", "axios 모듈 이용한 서버와의 API 통신", "props를 이용한 버튼 컴포넌트 재사용", "MUI를 활용한 컴포넌트 구현", "Webpack 번들링 및 alias 지정", "Cypress 테스트 자동화", "Github Actions CI/CD 활용한 자동배포"],
      troubles:['코드 및 디렉토리 리팩토링', '가로/세로/정방형 이미지 구분', '카카오맵 API 마우스오버 이벤트 부하 발생'],
      solutions: [
        '초반에 구현 자체에만 초점을 두고 코드를 작성한 결과, 코드, 변수명, 디렉토리 구성 등이 협업하기 어렵다는 문제 발생. 이에 협업을 위하여 대대적인 리팩토링을 실행하였음. 인라인 CSS 코드를 별도의 CSS 파일로 분리하였고, 직관적인 변수명을 사용하였으며, next.js 및 quasar 디렉토리 구조 참고하여 리팩토링 실행하였음. 추가적으로 공통적으로 사용되는 MUI 컴포넌트들은 props를 이용하여 재사용성 높임.',
    ...
    },
    {
        ...
        troubles: ['드래그앤드롭 구현 ', '드래그앤드롭 발생 시 여러 사용자에게서 동시에 동일한 동작이 발생하도록 구현해야 하는 문제', '카드가 없는 column은 드래그앤드롭이 동작하지 않는 문제.'],
        solutions: [
            ...
            '드래그앤드롭 발생 시, 모든 구성원들에게 동일한 카드이동이 발생해야 하고, 그룹 DB에는 한번만 반영이 되어야 했음. 이에 socket.io의 room 기반 통신을 활용하여, 드래그앤드롭이 발생할 경우 해당 이동정보를 socket 서버로 전송하고, socket 서버는 이를 나머지 구성원들에게 전송함으로써 모든 구성원이 동일한 이동정보를 수신할 수 있도록 함. 또한 직접 드래그앤드롭을 발생시켰을 경우와 다른 구성원이 드래그앤드롭을 발생시켰을 경우 setDataByColumnId 함수를 통해 동일한 동작이 발생하도록 하여 로직을 간소화하였음. socket 서버에 이동 정보가 도착했을 때 그룹 DB에 반영하도록 하여, DB에는 한번만 반영되도록 하였음',
      ...
      },
      {
        ...
        troubles: ['복잡한 로직으로 인한 전체적인 구조 파악의 어려움', 'Git repository 분할의 필요성'],
        solutions: [
      ...
      },
      {
        ...
        troubles: ['contentEditable의 중복 저장 문제', '텍스트 하이라이트 방식 좋아요 기능 구현', 'span 태그로 인한 selection API 추출 불가'],
        solutions: [
            '원래 텍스트 필드로 자유롭게 조작이 가능한 contentEditable를 사용하고자 했으나, contentEditable로 입력된 한글이 두 번씩 중복 저장되는 문제가 발생. 문제 원인을 찾아보았으나, 브라우저 동작 원리와 관련되어 있어 원인 자체의 해결이 불가하고 다만 중복 입력을 무시하는 방법 밖에 없었음. 이를 일부 구현하였으나, 안정성이 떨어져, textarea를 사용하여 텍스트 필드를 구현하였음.', 
      ...
      },
      {
        ...
        solutions: ['포트폴리오 페이지인만큼, 디자인이 심플하고 전달하고자 하는 내용이 정확히 전달되어야 했음. 이에 여러 레퍼런스들을 참고하여 최소한의 동작으로 페이지 간 이동이 가능하며, 에니메이션을 통해 자연스럽게 이동할 수 있고, 최대한 많은 요소에 마우스오버 및 클릭 이벤트를 적용함으로써 사용자 경험을 고려한 페이지를 구현함.']
      },
      {
        title: "Muse",
        ...
        troubles: ['MUI 컴포넌트와 SSR 방식이 호환되지 않는 문제', '함수 클로저로 인한 변경된 데이터의 미적용', 'props가 복잡해짐에 따라 interface 구현'],
        solutions: [
